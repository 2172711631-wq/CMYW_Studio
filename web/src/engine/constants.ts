...
// 先停在 4（= 关掉可变）。可变白本身是对的（色差 21.8 → 16.3、中性灰天生中性），
// 但白层会碎：一张线稿上裂成 960 块、919 块不到 8 格，全贴着线边 —— 白是底，
// 每块碎屑就是一小片多出来的厚度，透光看就是线边一圈亮度噪声。
// 按等值块压能到 330，压不干净（被高层包住的小块只能往上填）。
// 等 Lab 查表那一版：相近颜色落到同一条配方，白的选择天然连贯，不会碎。
export const MAX_WHITE_LAYERS = 4;

/** 抬浅层之前，先要求原图这一格**真的有颜色**：三通道极差（0..1）。
 *
 * 抬浅层是"需求不足半层也给它一整层"，本意是别让浅色整片消失。但门槛记在
 * 层数上，换算回原图只要 11/255 的偏色就够 —— 那个程度屏幕上根本看不见，
 * 却会拿到**整整一层品红**。实测一张插画：围裙、袜子、脚下影子里 2380 格
 * 就是这么变粉的，原色清一色是 254,243,247 这种。
 *
 * 那张图的直方图把两拨东西分得很干净：偏色 5–15/255 有 5.8 万格（围裙、
