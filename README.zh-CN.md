...
同一张源图、同一套光学模型、同一个透光模拟，**只有分色算法不同**。
左边是本项目自己的 `v1` 档案（朴素分色），右边是现在默认的 `v2`。

| 朴素 RGB → CMY 分色 | 自适应 UCR + 有序抖动 |
|---|---|
| <img src="docs/img/compare-naive.webp" width="330"> | <img src="docs/img/compare-ucr.webp" width="330"> |
| 中性色和暗部被三种颜色同时叠满，压成一片死黑；剩下的部分因为每通道只有 7 级，硬生生分出色阶带。 | 把三色共有的灰抽出来，再按像素自身亮度只补回一部分。暗部因此留得住细节，抖动把色阶带打散成细纹理。 |

暗部平均亮度（两边取同一批像素）：**19.9 → 86.2**。

这一下"压死"就是彩色透光画一直被说浑浊的原因。实物上它不会显示成纯黑 ——
三种饱和耗材叠在中性色上，出来是**一层灰褐色的雾罩住整幅画**。
把这层灰抽掉是差别的主要来源，顺带还省掉大量本来要叠三次的耗材。

自己复现：`py -3.11 tools/make_comparison.py 你的照片.jpg`

> 说清楚这组图比的是什么：**本项目自己的两个档案**。
> 我没有反编译过任何别家的生成器，所以这不是对其它工具内部实现的断言。
...
- [x] **浏览器版**——整个引擎移植到 TypeScript，在你自己的电脑或手机上跑，
...
发邮件到 **2172711631@qq.com**，标题写「商业授权」，说说你想做什么就行。
正式授权档位还在完善中、暂未开售，目前逐案处理。范围与边界见 [COMMERCIAL.md](COMMERCIAL.md)。
...
为 Bambu Lab X1C + AMS 而做。那组光学常数是拓竹 PLA Basic 上的工作值——
不是仪器实测，只是在大量图片上打出来都站得住的一组数。
如果你想为自己的耗材推一套，`tools/make_wedge.py` 和 `tools/measure_wedge.py`
会替你做拟合；推出来欢迎分享。
